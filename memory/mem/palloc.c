...
palloc_get_multiple (size_t page_cnt)
{
	...
	if(pages == NULL){
		pages = (void*)(VKERNEL_HEAP_START + page_alloc_index * PAGE_SIZE);//0xc000 0000 + index* 4KB
		page_alloc_index += page_cnt;//++;
	}

	if (pages != NULL) 
	{
		memset (pages, 0, PAGE_SIZE * page_cnt);// 4KB * number of page size memory reset
	}

	return (uint32_t*)pages; //return pages 
}
